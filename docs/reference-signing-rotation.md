...
Do not switch the signer before an overlap-capable Android release is deployed, and do not remove the old key before the first new-key publication succeeds. Publisher validation, the Android build, and the Android release gate all consume the same checked-in trust manifest, so source comments or dead Kotlin declarations cannot expand the release trust boundary.
