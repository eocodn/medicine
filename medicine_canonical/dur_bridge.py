...
def materialize_dur_ingredient_bridge(
    ...
    try:
        ...
        for row in product_rows:
            resolver.add(
                row["category"], row["ingredient_name_en"], row["ingredient_code"]
            )
            resolver.add(
                row["category"], row["paired_ingredient_name_en"], row["paired_ingredient_code"]
            )
        ...
        pair_signatures = 0
        for row in con.execute(
            """SELECT i.id,i.category,i.rule_value,
            ...
            if not explicit_left_code:
                raise ValueError(
                    f"MFDS criterion {criterion_id} is missing its authoritative ingredient code"
                )
            ...
            if category == COMBINATION_CATEGORY:
                ...
                con.execute(
                    """INSERT OR IGNORE INTO dur_criterion_pair_signatures(
                           criterion_rule_id,left_signature_key,right_signature_key,
                           match_method,evidence_kind
                       ) VALUES(?,?,?,'mfds_ingredient_code','mfds_criterion_code')""",
                    (
                        criterion_id,
            ...
            con.execute(
                """INSERT OR IGNORE INTO dur_criterion_signatures(
                       criterion_rule_id,category,effect_key,signature_key,
                       match_method,evidence_kind
                   ) VALUES(?,?,?,?,'mfds_ingredient_code','mfds_criterion_composition')""",
                (criterion_id, category, effect_key, signature_key(composition)),
            )
