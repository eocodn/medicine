(function attachMedicineOcrReview(root, factory) {
  ...
  function selectCandidate(row, candidate, options = {}) {
    ...
    if (!row || !candidate?.product_ref) return false;
  ...
  async function searchRow(rowId) {
    ...
    else if (row.candidates.length === 1) selectCandidate(row, row.candidates[0], { rerender: false });
    render();
  ...
  function candidateHtml(row) {
    ...
    return `<div class="ocr-product-candidates">${row.candidates.map((candidate) => `
      <button type="button" data-ocr-candidate="${escapeHtml(candidate.product_ref)}" data-ocr-row="${escapeHtml(row.row_id)}">
  ...
  function rowHtml(row, index) {
    ...
    return `<article class="ocr-review-row" data-ocr-row-card="${escapeHtml(row.row_id)}">
      <div class="ocr-row-heading"><div><span>약 ${index + 1}</span><small>${escapeHtml(associationCopy)}</small></div>
        ${state.rows.length > 1 ? `<button type="button" class="text-button" data-ocr-remove="${escapeHtml(row.row_id)}">행 삭제</button>` : ""}</div>
      <label class="ocr-field-wide">약 제품
        <div class="ocr-product-search"><input data-ocr-field="product_query" data-ocr-row="${escapeHtml(row.row_id)}" value="${escapeHtml(row.product_query)}" placeholder="제품명"><button type="button" data-ocr-search="${escapeHtml(row.row_id)}">찾기</button></div>
      </label>
      ${candidateHtml(row)}
      <div class="ocr-field-grid">
        <label>1회량<input type="number" min="0" step="0.1" data-ocr-field="dose_amount" data-ocr-row="${escapeHtml(row.row_id)}" value="${escapeHtml(row.dose_amount)}"></label>
        <label>단위<input data-ocr-field="dose_unit" data-ocr-row="${escapeHtml(row.row_id)}" value="${escapeHtml(row.dose_unit)}" placeholder="정, 캡슐, mL"></label>
        <label>1일 횟수<input type="number" min="1" max="24" data-ocr-field="frequency_per_day" data-ocr-row="${escapeHtml(row.row_id)}" value="${escapeHtml(row.frequency_per_day)}"></label>
        <label>처방 일수<input type="number" min="1" max="3650" data-ocr-field="prescription_days" data-ocr-row="${escapeHtml(row.row_id)}" value="${escapeHtml(row.prescription_days)}"></label>
      </div>
      <label class="ocr-field-wide">복용·사용 시간<input data-ocr-field="schedule_times" data-ocr-row="${escapeHtml(row.row_id)}" value="${escapeHtml(row.schedule_times)}" placeholder="08:00, 20:00"></label>
      <div class="ocr-field-grid">
        <label>식사 관계<select data-ocr-field="meal_relation" data-ocr-row="${escapeHtml(row.row_id)}">
        ...
        <label>투여 경로<select data-ocr-field="administration_route" data-ocr-row="${escapeHtml(row.row_id)}">
      ...
      <label class="ocr-field-wide">시작일<input type="date" data-ocr-field="start_date" data-ocr-row="${escapeHtml(row.row_id)}" value="${escapeHtml(row.start_date)}"></label>
      <label class="ocr-prn"><input type="checkbox" data-ocr-field="as_needed" data-ocr-row="${escapeHtml(row.row_id)}"${row.as_needed ? " checked" : ""}><span>필요할 때만 사용</span></label>
      ${assessmentHtml(row)}
    </article>`;
  }
  ...
  function bindEvents(rootNode) {
    ...
    rootNode.querySelector?.("#ocr-batch-cancel")?.addEventListener("click", () => config.close?.());
  }
  ...
  function render() {
    ...
    rootNode.innerHTML = `
      <div class="sheet-header ocr-review-header"><div><p class="eyebrow">OCR REVIEW</p><h2 id="ocr-review-title">인식한 처방을 확인해주세요</h2></div><button type="button" class="icon-button" id="ocr-batch-cancel">×</button></div>
      ...
      <button type="button" class="secondary-button wide" id="ocr-add-row">+ 약 한 줄 추가</button>
  ...
  async function createReviewedBatch(personId, rows) {
    const body = {
    ...
    try {
      const result = await config.api(`/api/people/${personId}/medications/batch`, { method: "POST", body: JSON.stringify(body) });
  ...
  async function submit() {
    ...
    try {
      ...
      if (state.phase !== "reviewed" && state.phase !== "ready") {
        const preview = await config.api(`/api/people/${personId}/medications/batch-preview`, {
          method: "POST",
      ...
      return await createReviewedBatch(personId, rows);
    } finally {
      state.busy = false;
