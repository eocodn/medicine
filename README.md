...
Android의 `MEDICINE_REFERENCE_UPDATE_RELEASE_BASE_URL`은 signed APK 빌드에서 기본 development endpoint를
다른 `r2.dev` channel로 교체할 때만 사용하며,
`MEDICINE_REFERENCE_UPDATE_BASE_URL`은 Android와 standalone web 모두에서 명시적 테스트 override로 사용할 수
있습니다. 직접 APK 배포에는 별도 production hostname이 필요하지 않습니다.
...
GitHub Release에 게시합니다. 별도 release key로 직접 배포용 signed APK가 필요하면 위의
`scripts/android_release_build.sh` 경로를 사용하고, 버전 변경과 태그 순서는 `docs/android-releasing.md`를 따릅니다.
