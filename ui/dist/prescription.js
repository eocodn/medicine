...
function setScheduleTimeControls(root, values) {
    ...
    if (hidden)
        hidden.value = displayValues.filter(Boolean).join(", ");
    if (!list)
        return;
    ...
    scheduleTimeInputs(root).forEach((input) => input.addEventListener("input", () => {
        if (hidden)
            hidden.value = scheduleTimeValues(root).join(", ");
...
function riskStatusHeading(preview) {
    ...
    if (hitCount)
        return `DUR 주의사항 ${hitCount}건이 있어요`;
    if (unknownCount)
        return `확인이 필요한 DUR 항목 ${unknownCount}건이 있어요`;
    if (hasClearDurCoverage(preview))
        return "DUR 주의사항 없음";
    return "DUR 판정 결과를 확인할 수 없어요";
}
function permitStatusNoticeHtml(product, existingMedication = false) {
    if (!product.permit_status || product.permit_status === "active")
        return "";
...
async function confirmEditMedication() {
    ...
    try {
        ...
        if (handleConfirmationRequired(error, "confirm-edit-med"))
            return;
        toast(error.message);
        return;
    }
...
async function confirmAddMedication() {
    if (!state.pendingProduct || !state.currentPersonId)
        return;
    const draft = prescriptionPayloadFromForm();
    ...
    if (state.reviewedDraftKey !== draftKey) {
        let reviewRequired;
        try {
            reviewRequired = await reviewPrescriptionDraft(state.pendingProduct.product_ref, draft, "confirm-add-med");
        }
        catch (error) {
            toast(error.message);
            return;
    ...
    try {
        ...
        if (handleConfirmationRequired(error, "confirm-add-med"))
            return;
        toast(error.message);
        return;
    }
...
async function reviewPrescriptionDraft(productRef, draft, buttonId) {
    ...
    if (button)
        button.textContent = reviewRequired ? "경고를 확인했고 계속 저장" : "저장 중...";
    return reviewRequired;
}
function handleConfirmationRequired(error, buttonId) {
    if (error.status !== 409 || !error.body?.confirmation_required)
        return false;
    ...
    const button = $(`#${buttonId}`);
    if (button)
        button.textContent = "경고를 확인했고 계속 저장";
