...
function profileMeta(person) {
    ...
    if (person.sex !== "male") {
        if (person.pregnancy_status === "pregnant")
            parts.push(pregnancyLabel(person.pregnancy_status));
        if (person.lactation_status === "breastfeeding")
            parts.push(lactationLabel(person.lactation_status));
    }
    if (person.profile_needs_review)
        parts.push("정보 확인 필요");
    return parts.join(" · ");
}
...
function updateBirthDayOptions(preferredDay = "") {
    ...
    if (previousDay && Number(previousDay) <= maxDay)
        form.elements.birth_day.value = previousDay;
}
...
function bindPeopleEvents() {
    ...
    document.addEventListener("medicine:sheet-closed", (event) => {
        ...
        if (detail?.id === "delete-person-sheet")
            state.pendingDeletePersonId = null;
    });
}
