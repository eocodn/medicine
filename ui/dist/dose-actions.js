...
async function recordPrnIntake(medicationId, button = null) {
    ...
    try {
        ...
        if (!MutationInvariants.isActiveOrigin(state.currentPersonId, personId))
            return;
        ...
        if (MutationInvariants.failureKind(error) === "definitive")
            clearPrnRequestId(medicationId);
        if (!MutationInvariants.isActiveOrigin(state.currentPersonId, personId))
            return;
        if (button) {
...
function queueDoseDesiredState(instanceId, desiredStatus, button = null) {
    ...
    if (applyPendingDoseIntent(instanceId, desiredStatus))
        renderHome();
    if (!entry.running)
        void drainDoseDesiredState(instanceId, entry);
}
async function drainDoseDesiredState(instanceId, entry) {
    if (state.doseMutations.get(instanceId) !== entry || entry.running)
        return;
    ...
    try {
        ...
        entry.running = false;
        if (state.doseMutations.get(instanceId) !== entry)
            return;
