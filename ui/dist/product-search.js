...
function renderProductSearchPage(page, { append = false } = {}) {
    ...
    if (append)
        ...
        root.innerHTML = cards || `<div class="empty-state"><strong>검색 결과가 없어요</strong>다른 제품명, 제조사 또는 성분명으로 검색해보세요.</div>`;
    if (page.has_more) {
...
function observeProductSearchMore(term, requestId) {
    ...
    if (!state.searchHasMore || typeof IntersectionObserver !== "function")
        return;
    ...
    state.searchObserver = new IntersectionObserver((entries) => {
        if (entries.some((entry) => entry.isIntersecting))
            void loadMoreProductSearch(term, requestId);
    }, { rootMargin: "240px" });
    state.searchObserver.observe(sentinel);
}
async function loadMoreProductSearch(term, requestId) {
    if (state.searchLoadingMore || !state.searchHasMore || state.searchNextOffset == null ||
    ...
    try {
        const page = assertProductSearchPage(await api(`/api/products?q=${encodeURIComponent(term)}&limit=30&offset=${offset}`, { coalesceKey: "product-search" }));
        if (requestId !== state.searchRequestId || state.searchTerm !== term ||
            $("#drug-query").value.trim() !== term)
            return false;
