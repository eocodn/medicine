...
from browser_ocr.document_parsing.real_data import load_real_source_manifest, prepare_real_annotation
...
def _batch_profile(args: argparse.Namespace, source_dataset_id: str) -> dict[str, Any]:
...
def run_real_batch(args: argparse.Namespace) -> dict[str, Any]:
    ...
    profile = _batch_profile(args, source.dataset_id)
    ...
    for index, sample in enumerate(source.samples, start=1):
        ...
        entries.append({"document_id": document_id, "annotation": annotation_path.name})
        _atomic_json(
            state_path,
