(function attachBrowserOcrParser(root, factory) {
  ...
  function regimenFields(value) {
    ...
    const text = String(value || "");
    const ambiguousPacketTablet = /\d+(?:\.\d+)?\s*포\s*\(\s*정\s*\)/u.test(text);
    ...
    return {
      ...
      // `1포(정)` is deliberately unresolved: the printed form does not prove
      // whether the intended administration unit is one packet or one tablet.
      dose_unit: ambiguousPacketTablet ? null : hints.dose_unit,
  ...
  function headerKey(value) {
    ...
    if (/총.*일수|(?:투약|투여|복용)일수/u.test(text)) return "days";
    return null;
  }
  ...
  function strictTypedFields(value) {
    const compact = String(value || "").replaceAll(" ", "");
    let match = compact.match(/^(\d+(?:\.\d+)?)(정|tablet|캡슐|capsule|포|mL|ml)$/iu);
    if (match) {
      const amount = Number(match[1]);
      const unit = match[2].toLowerCase();
      return { kind: "dose", fields: { dose_amount: amount, dose_unit: unit === "정" || unit === "tablet" ? "tablet" : unit === "캡슐" || unit === "capsule" ? "capsule" : unit === "포" ? "packet" : "mL" } };
    }
    match = compact.match(/^(\d+)회$/u);
    if (match) return { kind: "frequency", fields: { frequency_per_day: Number(match[1]) } };
    match = compact.match(/^(\d+)일(?:분)?$/u);
    if (match) return { kind: "days", fields: { prescription_days: Number(match[1]) } };
    return null;
  }

  function structuralRow(line) {
    const typed = line.items.map((item) => ({ item, parsed: strictTypedFields(item.text) })).filter((entry) => entry.parsed);
    if (new Set(typed.map((entry) => entry.parsed.kind)).size < 2) return null;
    const firstStructuredX = Math.min(...typed.map((entry) => entry.item.x1));
    const productItems = line.items.filter((item) => item.cx < firstStructuredX && !strictTypedFields(item.text));
    const product = cleanProduct(productItems.map((item) => item.text).join(""));
    if (!product) return null;
    const row = rowTemplate(product, "structural_table_row");
    const seen = new Set();
    for (const { parsed } of typed) {
      if (seen.has(parsed.kind)) return null;
      seen.add(parsed.kind);
      applyFields(row, parsed.fields, true);
    }
    return row;
  }

  function structuralRows(lines, headerIndex) {
    const result = [];
    let lastCy = lines[headerIndex]?.cy ?? 0;
    for (let index = headerIndex + 1; index < lines.length; index += 1) {
      const line = lines[index];
      if (headerAnchors(line).length) break;
      const row = structuralRow(line);
      if (!row) {
        if (result.length && line.cy - lastCy > Math.max(90, line.h * 3.5)) break;
        continue;
      }
      result.push(row);
      lastCy = line.cy;
  ...
  function unheadedStructuralRows(lines) {
    const rows = lines.map(structuralRow).filter(Boolean);
  ...
  function tableRows(lines) {
    for (let headerIndex = 0; headerIndex < lines.length; headerIndex += 1) {
      ...
      if (!anchors.some((item) => item.key === "product")) continue;
      const keys = new Set(anchors.map((item) => item.key));
      if (!["product", "dose", "frequency", "days"].every((key) => keys.has(key))) {
        const fallback = structuralRows(lines, headerIndex);
  ...
  function labeledProductLine(line) {
    const direct = labeledProduct(line.text);
    if (direct) return direct;
    const items = line.items || [];
    for (let width = 1; width <= Math.min(3, items.length); width += 1) {
      const label = items.slice(0, width).map((item) => item.text.replaceAll(" ", "")).join("");
      if (!/^(?:약명|제품명|약품명|의약품명)[:：]?$/u.test(label)) continue;
      const product = cleanProduct(items.slice(width).map((item) => item.text).join(""));
  ...
  function labeledRows(lines, medianHeight) {
    const productLines = lines.map((line, index) => ({ index, product: labeledProductLine(line) })).filter((item) => item.product);
    ...
    if (rows.length === 1) {
      const productIndex = productLines[0].index;
      const prior = [];
      for (let index = productIndex - 1; index >= 0; index -= 1) {
        const line = lines[index];
        if (lines[productIndex].cy - line.cy > Math.max(420, medianHeight * 10)) break;
        const fields = regimenFields(line.text);
        const families = Number(fields.dose_amount !== null) + Number(fields.frequency_per_day !== null)
          + Number(fields.prescription_days !== null);
        if (families >= 2) prior.push({ index, fields });
      }
      if (prior.length === 1) {
        applyFields(rows[0], prior[0].fields);
        consumed.add(prior[0].index);
      }
    }
    ...
    if (rows.length > 1) {
      for (let index = 0; index < lines.length; index += 1) {
        if (consumed.has(index) || labeledProductLine(lines[index])) continue;
  ...
  function parsePrescriptionDocument(items) {
    ...
    let unassociatedRegimen = false;
    if (!rows.length) rows = unheadedStructuralRows(lines);
    if (!rows.length) {
  ...
  function parseDocumentItems(items) {
    const parsed = parsePrescriptionDocument(items);
    const rows = parsed.rows.map((row, index) => {
      const draft = {};
      for (const key of ["dose_amount", "frequency_per_day", "prescription_days", "meal_relation", "administration_route", "as_needed"]) {
        if (row[key] !== null && row[key] !== undefined && row[key] !== false) draft[key] = row[key];
      }
      if (row.dose_unit) draft.dose_unit = row.dose_unit === "정" ? "tablet" : row.dose_unit === "캡슐" ? "capsule" : row.dose_unit === "포" ? "packet" : row.dose_unit;
      if (Array.isArray(row.schedule_times) && row.schedule_times.length) draft.schedule_times = row.schedule_times.slice();
      const uncertainty = [...parsed.ambiguity_codes];
      if (row.association === "unresolved" && !uncertainty.includes("UNRESOLVED_REGIMEN_ASSOCIATION")) uncertainty.push("UNRESOLVED_REGIMEN_ASSOCIATION");
      return {
        row_id: `ocr-row-${index + 1}`,
        product_query: row.product_query,
        draft,
        uncertainty_codes: uncertainty,
      };
    });
    return { rows, uncertainty_codes: [...parsed.ambiguity_codes, ...parsed.unsupported_codes] };
  }

  return { parsePrescriptionHints, parsePrescriptionDocument, parseDocumentItems };
});
