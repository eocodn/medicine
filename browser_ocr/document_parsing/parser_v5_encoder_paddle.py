...
class _ByteSequenceEncoder(nn.Layer):
    ...
    def forward(self, token_ids: paddle.Tensor, token_mask: paddle.Tensor) -> paddle.Tensor:
        ...
        return paddle.concat([mean_pool, max_pool], axis=1)
...
class ParserV5GlobalEncoder(nn.Layer):
    """Compact order-aware OCR encoder with dense relation-biased context.

    Text is consumed as the ordered UTF-8 byte sequence from Parser v5's
    stable model-input contract. Every OCR node can attend to every other OCR
    node, while directional geometry enters both the attention score and the
    relation value. No layout/template identifier or page-pixel branch exists.
    """
    ...
    def forward(self, tensors: ParserV5Tensors) -> tuple[paddle.Tensor, paddle.Tensor]:
        text = self.text_encoder(tensors.token_ids, tensors.token_mask)
