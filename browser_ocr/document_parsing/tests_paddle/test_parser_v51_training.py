...
class ParserV51TrainingTest(unittest.TestCase):
    ...
    def test_training_writes_direct_metrics_and_is_idempotent(self) -> None:
        with tempfile.TemporaryDirectory() as raw:
            ...
            self.assertIn("token_selection_precision", metrics)
            self.assertIn("token_selection_recall", metrics)
            self.assertIn("token_selection_f1", metrics)
            self.assertIn("field_token_exact_rate", metrics)
            self.assertGreater(metrics["selected_tokens_supervised"], 0)
            self.assertGreater(metrics["outside_tokens_supervised"], 0)
