...
use crate::medication_records::{Medication, RecordError};
...
#[derive(Debug)]
pub(crate) enum MedicationListError {
    BadRequest(String),
    NotFound(String),
...
pub(crate) fn handle_request(
    ...
    Some(match list(canonical_db, personal_db, person_id, raw_path) {
        Ok(body) => (200, body),
        Err(MedicationListError::BadRequest(detail)) => (400, json!({"detail": detail})),
        Err(MedicationListError::NotFound(detail)) => (404, json!({"detail": detail})),
        Err(MedicationListError::ReferenceUnavailable) => {
            (503, json!({"detail": "reference database unavailable"}))
        }
        Err(MedicationListError::Unavailable) => {
            (503, json!({"detail": "personal database unavailable"}))
        }
        Err(MedicationListError::Internal) => (500, json!({"detail": "unexpected server error"})),
    })
}
...
fn list(
    ...
    person_id: &str,
    raw_path: &str,
) -> Result<Value, MedicationListError> {
    let target = planning::target_date(raw_path).map_err(MedicationListError::from)?;
    let personal = personal_db::open(personal_db, Access::ReadOnly).map_err(map_open)?;
    people::load_person(&personal, person_id).map_err(MedicationListError::from)?;
    let medications = compose(canonical_db, &personal, person_id, target)?;
    Ok(Value::Array(
        medications
            .into_iter()
...
pub(crate) fn compose(
    ...
    person_id: &str,
    target: NaiveDate,
) -> Result<Vec<Medication>, MedicationListError> {
    let mut medications =
        load_active_medications(personal, person_id, target).map_err(MedicationListError::from)?;
    let canonical = canonical_db
        .map(|path| canonical_products::open(Some(path)))
        .transpose()
        .map_err(MedicationListError::from)?;
    if let Some(canonical) = canonical.as_ref() {
        for medication in &mut medications {
            enrich_medication(canonical, personal, person_id, medication, target)?;
        }
    }
    Ok(medications)
}

fn enrich_medication(
    canonical: &Connection,
    personal: &Connection,
    person_id: &str,
    medication: &mut Medication,
    target: NaiveDate,
) -> Result<(), MedicationListError> {
...
fn current_draft(medication: &Value) -> Result<Value, MedicationListError> {
    let object = medication
        .as_object()
        .ok_or(MedicationListError::Internal)?;
...
fn map_open(error: OpenError) -> MedicationListError {
    match error {
        OpenError::Unavailable => MedicationListError::Unavailable,
        OpenError::Sql => MedicationListError::Internal,
    }
}

impl From<PlanningError> for MedicationListError {
...
impl From<PeopleError> for MedicationListError {
...
impl From<RecordError> for MedicationListError {
...
impl From<AssessmentError> for MedicationListError {
...
impl From<ProductError> for MedicationListError {
