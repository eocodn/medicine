...
#[no_mangle]
pub extern "system" fn Java_com_medicine_android_MedicineNativeCore_nativeHandlesRequest(
    mut env: JNIEnv<'_>,
    _this: JObject<'_>,
    handle: jlong,
    method: JString<'_>,
    ...
    let result = catch_unwind(AssertUnwindSafe(|| {
        let method = required_string(&mut env, method)?;
        let path = required_string(&mut env, path)?;
        with_engine(handle, |engine| Ok(engine.handles_request(&method, &path)))
    }))
    .unwrap_or_else(|_| Err("native request routing panicked".to_owned()));
    match result {
        Ok(true) => JNI_TRUE,
        Ok(false) => JNI_FALSE,
        Err(error) => {
