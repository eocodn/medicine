...
fn with_recent_logs(
    ...
    let logs = recent_logs(con, person_id, 20)?;
...
fn recent_logs(con: &Connection, person_id: &str, limit: i64) -> Result<Vec<Value>, DoseError> {
    ...
    let rows = statement
        ...
        .map_err(|_| DoseError::Internal)?;
    rows.map(|row| row.map_err(|_| DoseError::Internal))
        .collect()
}
