...
class WebApiTest(unittest.TestCase):
    ...
    def test_mobile_shell_is_served(self) -> None:
        ...
        self.assertIn('href="/static/styles.css?v=20260813b"', response.text)
        ...
        self.assertIn('src="/static/app.js?v=20260814a"', response.text)
    ...
    def test_ocr_review_contract_is_exposed_without_raw_artifacts(self) -> None:
        ...
        self.assertIn('id="ocr-review-sheet"', page.text)
        self.assertIn('id="ocr-review-content"', page.text)
        self.assertIn('src="/static/ocr-review.js?v=20260814a"', page.text)
        self.assertIn("브라우저 안에서 사진을 인식할 수 있어요. 사진은 서버로 전송되지 않아요.", page.text)
        self.assertNotIn("기기 내 OCR 기능을 확인하는 중", page.text)
        self.assertIn('src="/static/ocr.js?v=20260813b"', page.text)
        ...
        self.assertIn("MedicineOcrReview.open", app.text)
        self.assertIn("MedicineOcrReview.init", app.text)
        self.assertIn("MedicineOcr.getReview", app.text)
        self.assertIn("MedicineOcr.clearReviewToken", app.text)
        self.assertIn("suggested_administration_route", app.text)
        self.assertIn("투여 경로를 확인해주세요", app.text)
    ...
    def test_browser_ocr_is_self_hosted_and_keeps_images_out_of_app_requests(self) -> None:
        ...
        self.assertIn("medications/batch-preview", review.text)
        self.assertIn("medications/batch", review.text)
        self.assertIn("전체 확인하고 일괄 등록", review.text)
        self.assertIn("data-ocr-field", review.text)
